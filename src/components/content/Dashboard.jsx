...
const dummyData = {
  ...
  playlists: [
    {
      ...
      videos: [{ thumbnail: "https://via.placeholder.com/150" }],
      createdAt: new Date(),
      isPublic: true,
    },
  ],
  ...
  watchHistory: [
    {
      id: "1",
      title: "Watch History Dummy Video",
      thumbnail: "https://via.placeholder.com/150",
      createdAt: new Date(),
    },
  ],
...
function Dashboard() {
  ...
  const fetchDashboardData = useCallback(
    async (showLoading = true, forceRefresh = false) => {
      ...
      try {
        ...
        const dashboardVideosResponse = await api.get(
          `/api/v1/videos/user/id/${userId}`,
          {
            signal,
          }
        );
        ...
        const watchHistoryResponse = await api
        ...
        const formattedWatchHistory =
          watchHistoryResponse.data?.data?.results.map((item) => ({
            id: item.video?.id,
        ...
        setDashboardData((prevData) => ({
          ...prevData,
          subscribers:
            subscribersResponse.data?.data?.subscribers ||
            dummyData.subscribers,
  ...
  useEffect(() => {
    if (location.state?.user || (user && isInitialLoad)) {
      fetchDashboardData(true, true).then(() => setIsInitialLoad(false));
    }
  ...
  const toggleVideoPublishStatus = async (videoId) => {
    try {
      ...
      toast.success(
        response.data?.message || "Video status updated successfully"
      );
    } catch (error) {
  ...
  const renderItemCard = (item, type, actions = []) => (
    <div
      ...
      style={{
        cursor:
          type === "video" || type === "playlist" || type === "tweet"
            ? "pointer"
            : "default",
      }}
    >
    ...
    <div className="flex h-screen w-full bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      <div className={`flex flex-col flex-1  `}>
        ...
        >
          ...
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-2xl p-6 shadow-2xl transform transition-all hover:scale-[1.02]">
          ...
          >
            <TabsList className="bg-white/80 backdrop-blur-xl shadow-lg border border-gray-200 rounded-2xl w-full grid grid-cols-7 gap-2">
              {[
                ...
                {
                  name: "Playlists",
                  stat: dashboardData.playlists.length || 0,
                },
                {
                  name: "Subscribers",
                  stat: dashboardData.subscribers.length || 0,
                },
                { name: "Tweets", stat: dashboardData.tweets.length || 0 },
                {
                  name: "History",
                  stat: dashboardData.watchHistory.length || 0,
                },
              ].map(({ name, stat }) => (
                <TabsTrigger
                  key={name}
            ...
            >
              ...
              <CardContent className="space-y-4">
                {paginatedVideos?.length > 0
                  ...
                  : renderEmptyState("videos", "Upload Your First Video", () =>
            ...
            >
              ...
              <CardContent className="space-y-4">
                {paginatedLikedVideos.length > 0
                  ? paginatedLikedVideos.map((video) =>
                      renderItemCard(video, "video")
                    )
                  : renderEmptyState("liked videos", "Browse Videos", () =>
                      navigate("/explore")
                    )}
            ...
            >
              ...
              <CardContent className="space-y-4">
                {paginatedComments && paginatedComments.length > 0
                      ...
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div
                            ...
                            onClick={() =>
                              navigate(`/video/${comment.videoId}`)
                            }
                          >
                            <img
                              src={
                  ...
                  : renderEmptyState("comments", "Browse Videos", () =>
                      navigate("/explore")
                    )}
                {comments.length > 5 && (
                  <Pagination
                            ...
                            <div className="flex space-x-1">
                              ...
                              <Button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  console.log(
                                    `Toggle playlist ${playlist.id} privacy`
                                  );
                                }}
                              >
                                {playlist.isPublic ? (
            ...
            >
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {paginatedSubscribers.length > 0
                  ...
                  : renderEmptyState("subscribers", "Create More Content", () =>
                      navigate("/VideoUpload")
                    )}
                ...
                {paginatedTweets.length > 0
                  ? paginatedTweets.map((tweet) => {
                      const tweetWithImage = {
                        ...tweet,
                        thumbnail: tweet.image || tweet.owner?.avatar,
                      };

                      return renderItemCard(tweetWithImage, "tweet", [
                        {
                  ...
                  : renderEmptyState("tweets", "Create Your First Tweet", () =>
            ...
            >
              ...
              <CardContent>
                {watchHistory && watchHistory.length > 0 ? (
                  ...
                  renderEmptyState("watch history", "Browse Videos", () =>
                    navigate("/explore")
                  )
                )}
              </CardContent>
